...
### Used Hardware Components (5G Version)
| Component | Model | Cost |
| --- | --- | ---: |
| Single-Board Computer | [Raspberry Pi 4 Model B, 8GB RAM](https://www.raspberrypi.com/products/raspberry-pi-4-model-b/) | 80€ |
| Power Supply | [Official Raspberry Pi Power Supply](https://www.raspberrypi.com/products/type-c-power-supply/) | 10€ |
| SD Card | Samsung EVO Plus 256GB (MB-ME256SA/EU) | 20€ |
| Modem | [Quectel RM520N-GL (M.2)](https://techship.com/product/quectel-rm520n-gl-5g-sub-6-m2/) | 260€ |
| Modem Adapter | [Sixfab Raspberry Pi 5G Development Kit (including Case, Antennas)](https://sixfab.com/product/raspberry-pi-5g-development-kit-5g-hat/) | 235€ |
| SIM Adapter | [Selfmade SIM PCB](sim-adapter/) | 2€ |
| Misc | Network Cable, Wires, etc. | 3€ |


Further remarks:
* Our first prototype used the Quectel RM500Q modem, which also performed reliably. However, because that model has since been discontinued, we upgraded to the more current Quectel RM520N-GL for the production version.
* Supporting other modems should be easily possible, because we use ModemManager as an abstraction layer to communicate with the attached modem. Similarily, other 5G HATs (e.g., [SIM8200EA-M2 5G HAT](https://www.waveshare.com/wiki/SIM8200EA-M2_5G_HAT)) should work equally well, since the modem is actually just attached via USB. However, we currently use HAT-specific GPIO ports to send out-of-band control messages to the modem (e.g., powering up/down via pulling a specific pin to high/low). Using a different HAT therefore requires minor patches to correctly map this to the corresponding GPIO pins.


### Assembly Instructions (5G Version)
<p align="left">
    <img alt="Current MobileAtlas Measurement Probe Prototype" title="Current MobileAtlas Measurement Probe Prototype" src="../images/probe_v5G_0_exploded.jpg" width="550">
</p>

1. **Remove Unused Antenna:** The case includes five antennas, but the modem provides only four antenna sockets. Remove the redundant antenna (the one located next to the fan).
2. **Prepare the Raspberry Pi:**
   1. Insert the SD card into the RPi.
   2. Attach the heat sinks to the CPU, RAM, and USB controller of the RPi.
   3. Install the pin spacers onto the RPi's 40-pin header.
3. **Mount the Raspberry Pi in the Case:** Screw the RPi into the case according to the case design, as shown [here](../images/probe_v5G_0_step_1.jpg).
4. **Prepare the Modem Adapter:**
   1. Apply the thermal pad to the underside of the modem.
   2. Insert the modem into the adapter/HAT.
   3. Secure the modem to the adapter using the plastic screw.
5. **Install Standoffs:** Screw the standoffs on top of the RPi. These will support the modem adapter.
6. **Add the 40-Pin Extender:** Place the 40-pin GPIO extender onto the RPi header.
7. **Install the M.2 Modem Adapter / HAT:**
   1. Route the fan cables between the standoff and the pin extender, then up through the modem adapter.
   2. Carefully plug the modem adapter HAT onto the 40-pin extender.
8. **Connect Fan and Secure the Adapter:**
   1. Plug the fan cables into the appropriate header on the HAT.
   2. Screw the modem adapter down into the standoffs.
   3. Install the USB bridge.
9. **Finalize Modem Assembly:**
   1.  Connect the antenna cables to the modem.
   2.  Insert the SIM PCB into the modem adapter's SIM slot (see [here](../images/probe_v5G_0_step_2.jpg))
   3.  Attach the modem heat sink.
10. **Route Power Cables:** Organize and route the RPi and modem power cables (see [here](../images/probe_v5G_0_step_3.jpg)). Finally, attach the transparent top cover of the case and add the MobileAtlas sticker.
    

## Probe Version 4G.0
<p align="left">
    <img alt="Current MobileAtlas Measurement Probe Prototype" title="Current MobileAtlas Measurement Probe Prototype" src="../images/probe_v4G_0.jpg" width="550">
</p>

### Used Hardware Components (4G Version)
| Component | Model | Cost |
| --- | --- | ---: |
